  ...
  if(isset($_SESSION['session_admin'])){
    ...
    ?>
      ...
      <head>
        ...
        <title> UI-ALPerf | Admin | All Reports </title>
      ...
      <body>
        ...
        <main id="main" class="main">
          ...
          <div class="pagetitle d-flex justify-content-between">
                ...
                <ol class="breadcrumb">
                  ...
                  <?php
                    if(isset($_GET['school_year'])){
                      $sy = $_GET["school_year"];
                      $school_year = mysqli_query($conn, "SELECT * FROM `class_report_info` WHERE `school_year` = '$sy'");
                      $fetch = mysqli_fetch_assoc($school_year);
                      ?><li class="breadcrumb-item active">School Year: <?php echo $fetch['school_year'] ?></li><?php
                    }
                  ?>
                </ol>
              </nav>
            ...
            <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="allreports">
                ...
                <li>
                  <?php
                    $current_sy = isset($_GET['school_year']) ? $_GET['school_year'] : '';
                    ...
                    while($row = mysqli_fetch_assoc($sy)){
                      $active = $current_sy == $row['school_year'] ? 'active' : '';
                      ?><a class="dropdown-item fw-bold d-flex align-items-center <?php echo $active; ?>" href="admin_all_reports.php?school_year=<?php echo $row['school_year'] ?>">SY: <?php echo $row['school_year'] ?></a><?php
                    }
                  ?>
                </li>
              </ul>
          ...
          <section class="section cat_reports">
            <div class="row">
              <div class="col-12 catt">
                <div class="card cardo">
                  <div class="card-body pt-2">
                    <ul class="nav nav-tabs">
                      <li class="nav-item">
                        <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#first_semester">First Semester</button>
                      </li>
                      <li class="nav-item">
                        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#second_semester">Second Semester</button>
                      </li>
                    </ul>
                    <div class="tab-content pt-3">
                      <div class="tab-pane fade show active first_semester" id="first_semester">
                        <div class="row">
                          <?php
                            if(isset($_GET['school_year'])){
                              $sy = $_GET["school_year"];
                              ...
                              WHERE info.admin_id = '$admin_ID' AND info.admin_archive = TRUE AND info.school_year = '$sy' AND info.semester = 'First Semester' ORDER BY info.report_info_id DESC");
                                      ...
                                      <div class="card cardee">
                                        <div class="card-header d-flex justify-content-between align-items-start">
                                          <div class="d-flex align-items-center herds">
                                            <div class="picsur">
                                              <img src="<?php echo $faculty_pic; ?>" width="100px">
                                            </div>
                      ...
                      <div class="tab-pane fade second_semester" id="second_semester">
                        <div class="row">
                          <?php
                            if(isset($_GET['school_year'])){
                              $sy = $_GET["school_year"];
                              ...
                              LEFT JOIN `class_report_ins` ins ON info.report_info_id = ins.report_ins_id
                              LEFT JOIN `class_report_com` com ON info.report_info_id = com.report_com_id
                              WHERE info.admin_id = '$admin_ID' AND info.admin_archive = TRUE AND info.school_year = '$sy' AND info.semester = 'Second Semester' ORDER BY info.report_info_id DESC");
                              if(mysqli_num_rows($class_report) > 0){
                                while($row = mysqli_fetch_assoc($class_report)){
                                  $faculty_ID = $row['faculty_id'];
                                  $check_faculty_pic = mysqli_query($conn, "SELECT `faculty_pic_path` FROM `faculty_pic` WHERE `id` = '$faculty_ID'");
