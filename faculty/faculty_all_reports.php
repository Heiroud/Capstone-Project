  ...
  if(isset($_SESSION['session_faculty'])){
    ...
    ?>
      ...
      <head>
        ...
        <title> UI-ALPerf | Faculty | All Reports </title>
                ...
                <li>
                  <?php
                    ...
                    while($row = mysqli_fetch_assoc($sy)){
                      ...
                      ?><a class="dropdown-item fw-bold d-flex align-items-center <?php echo $active; ?>" href="faculty_all_reports.php?school_year=<?php echo $row['school_year'] ?>"><?php echo $row['school_year'] ?></a><?php
                    }
                  ?>
                </li>
              </ul>
                        ...
                        <div class="row">
                          <?php
                            if(isset($_GET['school_year'])){
                              ...
                              WHERE info.faculty_id = '$faculty_ID' AND info.faculty_archive = TRUE AND info.school_year = '$sy' AND info.semester = 'First Semester' ORDER BY info.report_info_id DESC");
                        ...
                        <div class="row">
                          <?php
                            if(isset($_GET['school_year'])){
                              ...
                              LEFT JOIN `class_report_ins` ins ON info.report_info_id = ins.report_ins_id
                              LEFT JOIN `class_report_com` com ON info.report_info_id = com.report_com_id
                              WHERE info.faculty_id = '$faculty_ID' AND info.faculty_archive = TRUE AND info.school_year = '$sy' AND info.semester = 'Second Semester' ORDER BY info.report_info_id DESC");
                              if(mysqli_num_rows($class_report) > 0){
                                while($row = mysqli_fetch_assoc($class_report)){
                                  $admin_ID = $row['admin_id'];
                                  $check_admin_pic = mysqli_query($conn, "SELECT `admin_pic_path` FROM `admin_pic` WHERE `id` = '$admin_ID'");
